...
// EUI Key for synchronize data to AWS3
const char *appEui = "dca632fffe7bc6e5";
const char *devEui = "7e372d96c4200537";
const char *appKey = "c1b658d61c28225f7a9b31466529a440";
...
#define TX_INTERVAL 15       // default 5
#define TIME_ZONE (+7)
...
static void gps_event_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    switch (event_id)
    {
        case GPS_UPDATE:
            ...
            distance = hav(11.57112, 104.89695,gps->latitude, gps->longitude);

            int snr_data = LMIC.snr;
            int n_sat = gps->sats_in_use;

            float *coordinate = malloc(3 * sizeof(float_t));
...
void sendMessages(void* pvParameter)
{
    ...
    while (1)
    {
        ...
        if(xQueueReceive(send_msg_queue, &coordinate, portMAX_DELAY))
        {
            ...
            snprintf(buf, sizeof(buf), "{\"lat\":%.5f,\n \"long\":%.5f,\n \"dis\":%.3f,\n \"M\":%d}", latitude2, longitude2, distance2, count_button);
...
void Display_LVGL(void * param)
{
    ...
    lv_obj_align(rssi_data, LV_ALIGN_LEFT_MID, 83,10);
    ...
    while (1)
    {
        if (xQueueReceive(display_msg_queue, &receiver_coord, portMAX_DELAY) == pdPASS)
        {
            ...
            printf("lon = %.5f, lat = %.5f, dis = %.3f, snr = %.2f\n", receiver_coord[0], receiver_coord[1], receiver_coord[2], receiver_coord[3]);
...
void app_main(void)
{
    ...
    if (ttn_join())
    {
        ...
        xTaskCreate(sendMessages, "send_messages",8 * 1024, (void* )0, 3, NULL);   
